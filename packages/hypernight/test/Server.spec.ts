...
import { GlobalMiddlewareController } from '../test-infra/lib/controllers/GlobalMiddlewareController'
...
describe('Server', () => {
  ...
  describe('addControllers', () => {
    ...
    it('should be able to add controllers with global middleware', async () => {
      ...
      app.addControllers(
        new GlobalMiddlewareController(),
        undefined,
        globalMiddleware
      )
      server = app.start(port)
      await Promise.all([GlobalMiddlewareController.validateGlobalMiddleware()])
    })
